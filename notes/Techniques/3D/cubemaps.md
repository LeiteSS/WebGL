...
{{{example url="../cubemapFaces.html" }}}

Now let's apply that to a cube. We'll start with the code
from the texture atlas example [in the previous article](textures.md).
...
Recall from [the article on lighting](directional.md) that
...
{{{diagram url="diagram/cube-normals.html" caption="standard cube normal vs this cube's normals" }}}
...
to use a texture atlas like we [mentioned before](textures.md).
