...
In [the article on the drawing without data](drawingWithoutData.md)
...
with no math using the code [from the very first article](fundamentals.md).
...
{{{example url="../shadertoy-solid.html"}}}

In [the article on how WebGL works](howItWorks.md) we added more
color by providing a color for each vertex. In [the article on textures](textures.md)
...
{{{example url="../shadertoy-gl-fragcoord.html"}}}
...
{{{example url="../shadertoy-w-resolution.html"}}}
...
{{{example url="../shadertoy-w-mouse.html"}}}
...
look up the uniform, and set it in a [requestAnimationFrame loop](animation.md).
...
{{{example url="../shadertoy-w-time.html"}}}
...
to provide [textures](textures.md).
...
[textures to render to](render2Texture.md).
