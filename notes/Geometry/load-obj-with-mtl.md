...
Starting with the code [from the previous article](load-obj.md)
...
we'll add in the specular calculation from [the article on point lighting](point.md) except with one difference, we'll compute
...
{{{example url="../load-obj-w-mtl-no-textures.html"}}}
...
To load all these textures we'll use the code from [the article on textures](textures.md) slightly modified.
...
{{{example url="../load-obj-w-mtl-w-textures.html"}}}
...
We'll grab the code for generating tangents from [the article on normal mapping](normal-mapping.md).
...
And then we need to incorporate the shader changes from [the article on normal mapping](normal-mapping.md).
...
{{{example url="../load-obj-w-mtl-w-normal-maps.html"}}}
...
for [environment map](environmentMaps.md). They also show the various
...
* `-o u v w` specifies an offset for texture coordinates. You'd apply those using a texture matrix similar to what we did in [the article on drawImage](DrawImage.md)
