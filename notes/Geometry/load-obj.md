...
that is the combination of data of all 3 things. [*](pulling-vertices.md)
...
of the shaders from [the article on directional lighting](directional.md).
...
{{{example url="../load-obj-cube.html"}}}
...
[transparent](text-texture.md) and the bumpers are
[reflective](environmentMaps.md) so they need to be drawn differently
...
which we covered in [the article on cameras](Cameras.md).
...
for geometry in general you can look in [the article on lathing](geometry.md) for one example.
...
<div class="webgl_center"><img src="models/chair.jpg" style="width: 452px;"></div>
...
<div class="webgl_center"><img src="models/book.png" style="width: 446px;"></div>
...
can include alpha values for each vertex color. Our [helper library](fun.md) has been automatically taking the data we pass it and making buffers for
...
vertex colors. On [the first article](fundamentals.md) as well as
[other articles](Attributes.md) we covered that an attribute usually
...
Our [helper library](fun.md) handles this for us if
...
{{{example url="../load-obj-w-vertex-colors.html"}}}

As for parsing and using materials [see the next article](load-obj-with-mtl.md)
...
[the lathe example](geometry.md) we could generate normals
...
* No hierarchy, No scene graph
  ...
  because .OBJ contains no [scene graph](sceneGraphs.md). Better formats
...
* no support for animation or skinning
  ...
  We went over [skinning](skinning.md) elsewhere but .OBJ provides no
...
* .OBJ requires parsing
  ...
  You can see an example of loading a .GLTF file in [the article on skinning](skinning.md).
