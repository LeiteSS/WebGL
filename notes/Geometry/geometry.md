...
How to make a bowling pin shape in WebGL? The *smart* answer is: "Use a 3D modeling package like [Blender](https://blender.org),
...
But if they wanted to make a modeling package? One way is to make a cylinder and try to pinch it in
...
First, we need some way to make a curve.
Then we'd need to compute points on that curve. We'd then rotate
those points around some axis using [matrix math](2DMatrices.md)
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=0" }}}
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=1" }}}
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=2" }}}

Finally we do the same for those 2 points and compute the `t` point between
`R1 R2`.

{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=3" }}}

The positions of that <span style="color: red;">red point</span> make a curve.

{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=4" }}}
...
{{{diagram url="diagram/bezier-curve-edit.html" }}}
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=0&showCurve=true&showPoints=true" }}}
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=4&show2Curves=true" }}}
...
{{{diagram url="diagram/bezier-curve-diagram.html?maxDepth=0&showCurve=true&showTolerance=true" }}}
...
<img class="webgl_center" src="img/bowling-pin-silhouette.svg" width="50%" height="50%" />
...
you use the [matrix math](2DMatrices.md) to rotate the points around the Y axis.
...
Using our [simplified code](fun.md) we can generate WebGL buffers with
this data like this
...
<img class="webgl_center" src="img/tangent-to-curve.png" width="50%" />
...
<img class="webgl_center" src="img/candle-holder.svg" width="50%" />
...
<img class="webgl_center" src="img/lathe-normal-seam.png" width="50%" />
